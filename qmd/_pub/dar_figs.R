...
dar_1e6$pk_tu %>%
    ...
    mutate(
        ...
        point_group = fct_relevel(
            as.character(point_group),
            c("pt_17", "pt_14"),
        ),
    ...
    dplyr::arrange(desc(point_group)) %>%
    ggplot(aes(rel_position, dar_region)) +
    geom_point(aes(colour = point_group), data = . %>% dplyr::filter(point_group == "pt_other"), size = 0.5, show.legend = FALSE) +
    geom_smooth(aes(colour = line_group), method = "loess", data = . %>% dplyr::filter(line_group == "ln_other"), se = FALSE, linewidth = 0.8) +
    geom_point(aes(colour = point_group), data = . %>% dplyr::filter(point_group %in% c("pt_14", "pt_17")), size = 0.5, show.legend = FALSE) +
    geom_line(aes(colour = point_group), data = . %>% dplyr::filter(point_group %in% c("pt_14", "pt_17")), show.legend = TRUE) +
    geom_xsidevline(xintercept = de_midpoint, linetype = "solid", colour = pal_paired[2], linewidth = 0.7) +
    geom_ysidehline(yintercept = de_dar, linetype = "solid", colour = pal_paired[2], linewidth = 0.7) +
    ...
    scale_colour_manual(
        ...
        labels = c("pt_14" = "Chromosome 14", "pt_17" = "Chromosome 17", "ln_other" = "All other chromosomes"),
        breaks = c("pt_14", "pt_17", "ln_other")
    ) +
...
ggsave(
    "~/phd/publications/pktu_manuscript/fig/dar_pktu.png",
    width = 8, height = 4
)
...
a <- dar_1e6$pk_het %>%
    ...
    ggplot(aes(rel_position, dar_region)) +
    geom_point(aes(colour = point_group), data = . %>% dplyr::filter(point_group == "pt_other"), size = 0.5, show.legend = FALSE) +
    geom_smooth(aes(colour = line_group), data = . %>% dplyr::filter(line_group == "ln_other"), se = FALSE, linewidth = 0.8) +
    geom_point(aes(colour = point_group), data = . %>% dplyr::filter(point_group %in% c("pt_14", "pt_17")), size = 0.5, show.legend = FALSE) +
    geom_line(aes(colour = point_group), data = . %>% dplyr::filter(point_group %in% c("pt_14", "pt_17")), show.legend = TRUE) +
    geom_xsidevline(xintercept = de_midpoint, linetype = "solid", colour = pal_paired[2], linewidth = 0.7) +
    geom_ysidehline(yintercept = de_dar, linetype = "solid", colour = pal_paired[2], linewidth = 0.7) +
...
b <- dar_1e6$tu_het %>%
    ...
    geom_point(aes(colour = point_group), data = . %>% dplyr::filter(point_group %in% c("pt_14", "pt_17")), size = 0.5, show.legend = FALSE) +
    geom_line(aes(colour = point_group), data = . %>% dplyr::filter(point_group %in% c("pt_14", "pt_17")), show.legend = TRUE) +
    geom_xsidevline(xintercept = 0.5, linetype = "blank", colour = pal_paired[2], linewidth = 0.7) +
    geom_ysidehline(yintercept = 0.5, linetype = "blank", colour = pal_paired[2], linewidth = 0.7) +
    coord_cartesian(xlim = c(0, 1), ylim = c(0, 0.51)) +
    scale_colour_manual(
        values = c(
            "pt_14" = pal_paired[2],
...
ggsave(
    "~/phd/publications/pktu_manuscript/fig/dar_hets.png",
    width = 8, height = 8
)

## EofAD vs WT & fAI vs WT
...
de_dar <- as_tibble(gene_dar$eofad_wt) %>%
    left_join(as_tibble(tt$eofad_wt)[,c("gene_id", "chr", "DE")]) %>%
    dplyr::filter(DE, chr == 17) %>%
    pull(dar)
c <- dar_1e6$eofad_wt %>%
    as_tibble() %>%
    ...
    geom_text(x = psen1_midpoint - .05, y = 0.49, label = "psen1", fontface = "italic") +
    geom_xsidevline(xintercept = de_midpoint, linetype = "solid", colour = pal_paired[6], linewidth = 0.7) +
    geom_ysidehline(yintercept = de_dar, linetype = "solid", colour = pal_paired[6], linewidth = 0.7) +
    coord_cartesian(xlim = c(0, 1), ylim = c(0, 0.51)) +
    scale_colour_manual(
        values = c(
            "pt_14" = pal_paired[2],
...
ggsave(
    "~/phd/publications/pktu_manuscript/fig/dar_eofadwt_faiwt.png",
    width = 8, height = 8
)
...
dar_1e6$eofad_fai %>%
    ...
    ggside(x.pos = "bottom", y.pos = "left")
ggsave(
    "~/phd/publications/pktu_manuscript/fig/dar_eofadfai.png",
    width = 8, height = 4
)
